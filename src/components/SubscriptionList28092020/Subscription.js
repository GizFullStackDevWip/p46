...
const Subscription = ({ param }) => {
    /*const [subscription, setSubscription] = useState(param)
    useEffect(() => {
    ...
    const onSubscribeHandler = (param) => {
        const urlParams = new URLSearchParams(window.location.search);
        let antkn = urlParams.get('antkn');
        if(antkn) {
            service.androidTokeDecode(antkn).then(response => {
                console.log(response.data, 'dara');
                var data = response.data;
               // param and data pass to payment page.
            })
        }
    }
                            ...
                            <div className="movieTile mytitle" key={index} >
                                <div>
                                    <div className="moviePoster"style={{ padding: '20% 0'}}  >
                                        ...
                                        <button type="button" onClick ={() => onSubscribeHandler(param)} className="subscribe-btn"  style={{cursor:'pointer'}}>Subscribe</button>
                                    </div>
