...
function fetchPremiumSubscriptionList(currentPlan) {
  var token = localStorage.getItem("access-token");
  let ipaddress = getCookie("ipaddress");
  let deviceId = localStorage.getItem("deviceId");
  ...
  const customConfig = {
    ...
    params: {
      pubid: process.env.REACT_APP_PUBID,
      subscription_id: currentPlan
    },
  };
  return axios
    .get(process.env.REACT_APP_API_URL + "subscription/premium", customConfig)
    .then((response) => {
      return response.data;
    })
    .catch((error) => {
