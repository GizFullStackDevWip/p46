...
const EpisodeDetails = (props) => {
  ...
  useEffect(() => {
    ...
    service.getShowDetails(props.statesDetails.show.showId).then((response) => {
      ...
      dispatch({
        type: "SHOW_ID",
        payload: props.statesDetails.show.showId,
      });
      ...
      data.map((item, index) => {
        if (item.video_id !== props.statesDetails.show.show_details.video_id) {
          subItem.push(item);
        } else {
          setSelectedShowDetails(item);
        }
      });
      setCurrentSeason(subItem);

      //

      setEpisodeList(subItem);
      service.similarShow(props.statesDetails.show.showId).then((response) => {
        if (response.success == true && response.data.length > 0) {
          setSimilarShows(response.data);
        }
      });
      // data.map((item, index) => {
      //   if (item.video_id === props.statesDetails.show.video_id) {
      //     videoDetail = item;
      //     details = item;
      //   }
      // });
  ...
  const onClickPlay = (videoDetails, singleVideo, showId) => {
    ...
    if (user_id == null || user_id == service.getCookie("guestUserId")) {
      ...
      if (
                ...
                videoSubLists.map(function (subscription, index) {
                  if (useResponse.data.length == 0 && subFlag) {
                    ...
                    if (
                      ...
                      console.log(showId, singleVideo, videoDetails);
                      history.push({
        ...
        videoDetails.free_video === true &&
        videoDetails.subscriptions &&
        videoDetails.subscriptions.length === 0 //free video without subscription
      ) {
        localStorage.setItem("ContinueWatching", "true");
        history.push({
          pathname: "/videoplayer",
    ...
    if (path === "episode") {
      history.push({
        ...
        state: {
          show_details: show,
          singleVideo: "0",
          showId: props.statesDetails.show.showId,
        },
      });
    ...
    if (isLoggedIn === "true") {
      ...
      service.addToMyPlayList(show.show_id, 1).then((response) => {
        if (response.success === true) {
          service
            .getShowDetails(props.statesDetails.show.showId)
            .then((response) => {
              var data = response.data.videos;
              var videoDetail = "";
              setShowDetails(response.data);
              ...
              data.map((item, index) => {
                if (
                  item.video_id !==
                  props.statesDetails.show.show_details.video_id
                ) {
                  subItem.push(item);
                } else {
                  setSelectedShowDetails(item);
              ...
              setEpisodeList(subItem);
              service
                .similarShow(props.statesDetails.show.showId)
                .then((response) => {
                  if (response.success == true && response.data.length > 0) {
                    setSimilarShows(response.data);
                  }
  ...
  console.log("setSelectedShowDetails", selectedShowDetails);
  return (
                      ...
                      >
                        <div className="vpLeftButtons">
                          {showDetails.watchlist_flag === 1 ? (
                            <button
                              className="button buttonSecondary buttonBlock vpAddButton add__btn"
                              onClick={() => {
                          ...
                          <div style={{ width: "200px", marginBottom: "20px" }}>
                            <Dropdown
                              ...
                              onChange={(item) => {
                                ...
                                newSeason[0].videos &&
                                  newSeason[0].videos.map((item, index) => {
                                    if (
                                      item.video_id !==
                                      props.statesDetails.show.show_details
                                        .video_id
                                    ) {
                                      subItem.push(item);
                                    } else {
